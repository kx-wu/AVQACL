...
class IncreAudioVisualNet(nn.Module):
    def __init__(self, args, step_out_ans_num, vocab_size, LSC=False):
        ...
        if self.modality == 'visual':
            ...
            self.spatial_attn = nn.MultiheadAttention(512, 4, dropout=0.1)
            self.temporal_attn = nn.MultiheadAttention(512, 4, dropout=0.1)
    ...
    def forward(self, audio, visual, question, out_logits=True, out_features=False, out_features_norm=False, out_feature_before_fusion=False, que_feature=False):
        if self.modality == 'visual':
            ...
            visual_feat_att = \
                self.spatial_attn(xq, visual_feat, visual_feat, attn_mask=None, key_padding_mask=None)[0].squeeze(0)
            ...
            audio_feat_att = self.temporal_attn(xq, audio_feat, audio_feat, attn_mask=None, key_padding_mask=None)[
                0].squeeze(0)
