...
def train(args, task, step, train_data_set, val_data_set, exemplar_set):
    ...
    if task == 0 and step == 0:
        ...
        old_model.incremental_classifier(step_out_ans_num)
    ...
    for epoch in range(args.max_epoches):
        ...
        num_steps = 0
        model.train()
        if (task == 0 and step == 0) or (step_out_ans_num - last_step_out_class_num == 0):
        ...
        for samples in iterator:
            if (task == 0 and step == 0) or (step_out_ans_num - last_step_out_class_num == 0):
                ...
                labels = labels % (step_out_ans_num - last_step_out_class_num)
                labels = labels.to(device)
...
def detailed_test(args, task, step, test_data_set, task_best_acc_list):
    ...
    test_top1 = top_1_acc(all_test_out_logits, all_test_ans)     ###整体准确率计算
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='split_music_avqa', choices=['split_music_avqa', 'split_avqa'])
    parser.add_argument('--modality', type=str, default='audio-visual', choices=['visual', 'audio', 'audio-visual'])
    parser.add_argument('--audio_dir', type=str,
                        default='../../../features/split_music_avqa/vggish', help='audio feats dir')
    parser.add_argument('--visual_dir', type=str,
                        default='../../../features/split_music_avqa/resnet18', help='visual feats dir')
