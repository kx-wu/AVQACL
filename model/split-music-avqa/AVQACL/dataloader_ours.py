...
class IcreLoader(Dataset):
    def __init__(self, args, mode='train', modality='audio-visual', incremental_task=0, incremental_step=0):
        ...
        self.label_to_ix = json.load(open('../../../data/split_music_avqa/json/label_dict_11_2.json', 'r'))
        self.ques_word_to_ix = {}
        self.task_partition = json.load(open('../../../data/split_music_avqa/json/task_partition_11_2.json', 'r'))
        self.all_task = ['Counting', 'Existential', 'Location', 'Comparative', 'Temporal']
        self.all_ans_len = {}
        self.max_len = 14
    ...
    def num_current_step_qa(self):
        ...
        if self.mode == 'train':
            ...
            for sample in self.all_current_data_vids:
                ...
                if sample['anser'] not in current_ans_vocab:
                    current_ans_vocab.append(sample['anser'])
    ...
    def current_step_data(self):
        if self.mode == 'train':
            self.all_current_data_vids = []
            data = json.load(open(
                f'../../../data/split_music_avqa/json/{self.mode}_' + f'{self.all_task[self.incremental_task]}.json',
                'r'))
            for sample in data:
                common_label = [item for item in sample['label'] if item in self.task_partition[self.incremental_step]]
                if common_label:
                    sample['label'] = self.all_task[self.incremental_task] + '_' + common_label[0]
...
class exemplarLoader(Dataset):
    ...
    def _init_new_memory_class_exemplars_(self, new_memory_classes, exemplar_num_per_class):
        ...
        last_step_data = []
        data = json.load(open(
            f'../../../data/split_music_avqa/json/train_' + f'{self.all_task[self.incremental_task]}.json',
            'r'))
        for sample in data:

            common_label = [item for item in sample['label'] if item in self.task_partition[self.incremental_step-1]]
            if common_label:
                sample['label'] = self.all_task[self.incremental_task] + '_' + common_label[0]
